...
$party_work_req_role_add = NULL; // Initialize page object first

class cparty_work_req_role_add extends cparty_work_req_role {
	...
	// Page object name
	var $PageObjName = 'party_work_req_role_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["pwrr_id"] != "") {
				...
				$this->setKey("pwrr_id", $this->pwrr_id->CurrentValue); // Set up key
			} else {
				$this->setKey("pwrr_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "party_work_req_roleview.php")
	...
	function LoadDefaultValues() {
		$this->pwrr_id->CurrentValue = NULL;
		$this->pwrr_id->OldValue = $this->pwrr_id->CurrentValue;
		$this->fk_pwrr->CurrentValue = NULL;
		$this->fk_pwrr->OldValue = $this->fk_pwrr->CurrentValue;
		$this->fk_pwrr2->CurrentValue = NULL;
		$this->fk_pwrr2->OldValue = $this->fk_pwrr2->CurrentValue;
		$this->fk_pwrr3->CurrentValue = NULL;
		$this->fk_pwrr3->OldValue = $this->fk_pwrr3->CurrentValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->pwrr_id->EditValue = ew_HtmlEncode($this->pwrr_id->CurrentValue);
			$this->pwrr_id->PlaceHolder = ew_RemoveHtml($this->pwrr_id->FldCaption());
	...
	function AddRow($rsold = NULL) {
		...
		// pwrr_id
		$this->pwrr_id->SetDbValueDef($rsnew, $this->pwrr_id->CurrentValue, "", FALSE);

		// fk_pwrr
		$this->fk_pwrr->SetDbValueDef($rsnew, $this->fk_pwrr->CurrentValue, NULL, FALSE);

		// fk_pwrr2
		$this->fk_pwrr2->SetDbValueDef($rsnew, $this->fk_pwrr2->CurrentValue, NULL, FALSE);

		// fk_pwrr3
		$this->fk_pwrr3->SetDbValueDef($rsnew, $this->fk_pwrr3->CurrentValue, NULL, FALSE);
		...
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['pwrr_id']) == "") {
...
// Create page object
if (!isset($party_work_req_role_add)) $party_work_req_role_add = new cparty_work_req_role_add();

// Page init
$party_work_req_role_add->Page_Init();

// Page main
$party_work_req_role_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$party_work_req_role_add->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var party_work_req_role_add = new ew_Page("party_work_req_role_add");
party_work_req_role_add.PageID = "add"; // Page ID
var EW_PAGE_ID = party_work_req_role_add.PageID; // For backward compatibility

// Form object
var fparty_work_req_roleadd = new ew_Form("fparty_work_req_roleadd");

// Validate form
fparty_work_req_roleadd.Validate = function() {
...
fparty_work_req_roleadd.Form_CustomValidate = 
...
fparty_work_req_roleadd.ValidateRequired = true;
<?php } else { ?>
fparty_work_req_roleadd.ValidateRequired = false; 
<?php } ?>
...
<?php $party_work_req_role_add->ShowPageHeader(); ?>
<?php
$party_work_req_role_add->ShowMessage();
?>
<form name="fparty_work_req_roleadd" id="fparty_work_req_roleadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($party_work_req_role_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $party_work_req_role_add->Token ?>">
<?php } ?>
...
<input type="text" data-field="x_pwrr_id" name="x_pwrr_id" id="x_pwrr_id" size="30" maxlength="10" placeholder="<?php echo ew_HtmlEncode($party_work_req_role->pwrr_id->PlaceHolder) ?>" value="<?php echo $party_work_req_role->pwrr_id->EditValue ?>"<?php echo $party_work_req_role->pwrr_id->EditAttributes() ?>>
</span>
...
<script type="text/javascript">
fparty_work_req_roleadd.Init();
</script>
<?php
$party_work_req_role_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$party_work_req_role_add->Page_Terminate();
?>
