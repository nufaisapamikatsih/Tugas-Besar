...
$we_type_edit = NULL; // Initialize page object first

class cwe_type_edit extends cwe_type {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->wetype_id->CurrentValue == "")
			$this->Page_Terminate("we_typelist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// standard_work_hours
			$this->standard_work_hours->SetDbValueDef($rsnew, $this->standard_work_hours->CurrentValue, NULL, $this->standard_work_hours->ReadOnly);
...
// Create page object
if (!isset($we_type_edit)) $we_type_edit = new cwe_type_edit();

// Page init
$we_type_edit->Page_Init();

// Page main
$we_type_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$we_type_edit->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var we_type_edit = new ew_Page("we_type_edit");
we_type_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = we_type_edit.PageID; // For backward compatibility

// Form object
var fwe_typeedit = new ew_Form("fwe_typeedit");

// Validate form
fwe_typeedit.Validate = function() {
...
fwe_typeedit.Form_CustomValidate = 
...
fwe_typeedit.ValidateRequired = true;
<?php } else { ?>
fwe_typeedit.ValidateRequired = false; 
<?php } ?>
...
<?php $we_type_edit->ShowPageHeader(); ?>
<?php
$we_type_edit->ShowMessage();
?>
<form name="fwe_typeedit" id="fwe_typeedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($we_type_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $we_type_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $we_type->wetype_id->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_wetype_id" name="x_wetype_id" id="x_wetype_id" value="<?php echo ew_HtmlEncode($we_type->wetype_id->CurrentValue) ?>">
