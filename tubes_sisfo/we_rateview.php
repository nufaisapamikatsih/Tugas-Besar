...
$we_rate_view = NULL; // Initialize page object first

class cwe_rate_view extends cwe_rate {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["werate_id"] <> "") {
			$this->RecKey["werate_id"] = $_GET["werate_id"];
			$KeyUrl .= "&amp;werate_id=" . urlencode($this->RecKey["werate_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["werate_id"] <> "") {
				...
				$this->RecKey["werate_id"] = $this->werate_id->QueryStringValue;
			} else {
				$sReturnUrl = "we_ratelist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "we_ratelist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "we_ratelist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($we_rate_view)) $we_rate_view = new cwe_rate_view();

// Page init
$we_rate_view->Page_Init();

// Page main
$we_rate_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$we_rate_view->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var we_rate_view = new ew_Page("we_rate_view");
we_rate_view.PageID = "view"; // Page ID
var EW_PAGE_ID = we_rate_view.PageID; // For backward compatibility

// Form object
var fwe_rateview = new ew_Form("fwe_rateview");

// Form_CustomValidate event
fwe_rateview.Form_CustomValidate = 
...
fwe_rateview.ValidateRequired = true;
<?php } else { ?>
fwe_rateview.ValidateRequired = false; 
<?php } ?>
...
<?php $we_rate_view->ExportOptions->Render("body") ?>
<?php
	foreach ($we_rate_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $we_rate_view->ShowPageHeader(); ?>
<?php
$we_rate_view->ShowMessage();
?>
<form name="fwe_rateview" id="fwe_rateview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($we_rate_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $we_rate_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="we_rate">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($we_rate->werate_id->Visible) { // werate_id ?>
	<tr id="r_werate_id">
		<td><span id="elh_we_rate_werate_id"><?php echo $we_rate->werate_id->FldCaption() ?></span></td>
		<td<?php echo $we_rate->werate_id->CellAttributes() ?>>
<span id="el_we_rate_werate_id" class="form-group">
<span<?php echo $we_rate->werate_id->ViewAttributes() ?>>
<?php echo $we_rate->werate_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($we_rate->work_task->Visible) { // work_task ?>
	<tr id="r_work_task">
		<td><span id="elh_we_rate_work_task"><?php echo $we_rate->work_task->FldCaption() ?></span></td>
		<td<?php echo $we_rate->work_task->CellAttributes() ?>>
<span id="el_we_rate_work_task" class="form-group">
<span<?php echo $we_rate->work_task->ViewAttributes() ?>>
<?php echo $we_rate->work_task->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($we_rate->fk_werate->Visible) { // fk_werate ?>
	<tr id="r_fk_werate">
		<td><span id="elh_we_rate_fk_werate"><?php echo $we_rate->fk_werate->FldCaption() ?></span></td>
		<td<?php echo $we_rate->fk_werate->CellAttributes() ?>>
<span id="el_we_rate_fk_werate" class="form-group">
<span<?php echo $we_rate->fk_werate->ViewAttributes() ?>>
<?php echo $we_rate->fk_werate->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($we_rate->fk_werate2->Visible) { // fk_werate2 ?>
	<tr id="r_fk_werate2">
		<td><span id="elh_we_rate_fk_werate2"><?php echo $we_rate->fk_werate2->FldCaption() ?></span></td>
		<td<?php echo $we_rate->fk_werate2->CellAttributes() ?>>
<span id="el_we_rate_fk_werate2" class="form-group">
<span<?php echo $we_rate->fk_werate2->ViewAttributes() ?>>
<?php echo $we_rate->fk_werate2->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($we_rate->from_date->Visible) { // from_date ?>
	<tr id="r_from_date">
		<td><span id="elh_we_rate_from_date"><?php echo $we_rate->from_date->FldCaption() ?></span></td>
		<td<?php echo $we_rate->from_date->CellAttributes() ?>>
<span id="el_we_rate_from_date" class="form-group">
<span<?php echo $we_rate->from_date->ViewAttributes() ?>>
<?php echo $we_rate->from_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($we_rate->thru_date->Visible) { // thru_date ?>
	<tr id="r_thru_date">
		<td><span id="elh_we_rate_thru_date"><?php echo $we_rate->thru_date->FldCaption() ?></span></td>
		<td<?php echo $we_rate->thru_date->CellAttributes() ?>>
<span id="el_we_rate_thru_date" class="form-group">
<span<?php echo $we_rate->thru_date->ViewAttributes() ?>>
<?php echo $we_rate->thru_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($we_rate->rate->Visible) { // rate ?>
	<tr id="r_rate">
		<td><span id="elh_we_rate_rate"><?php echo $we_rate->rate->FldCaption() ?></span></td>
		<td<?php echo $we_rate->rate->CellAttributes() ?>>
<span id="el_we_rate_rate" class="form-group">
<span<?php echo $we_rate->rate->ViewAttributes() ?>>
<?php echo $we_rate->rate->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
