...
$fixed_asset_edit = NULL; // Initialize page object first

class cfixed_asset_edit extends cfixed_asset {
	...
	// Page object name
	var $PageObjName = 'fixed_asset_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->fasset_id->CurrentValue == "")
			$this->Page_Terminate("fixed_assetlist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->fk_fasset->SetDbValueDef($rsnew, $this->fk_fasset->CurrentValue, NULL, $this->fk_fasset->ReadOnly);

			// name
			$this->name->SetDbValueDef($rsnew, $this->name->CurrentValue, NULL, $this->name->ReadOnly);

			// date_acquired
			$this->date_acquired->SetDbValueDef($rsnew, $this->date_acquired->CurrentValue, NULL, $this->date_acquired->ReadOnly);

			// last_service
			$this->last_service->SetDbValueDef($rsnew, $this->last_service->CurrentValue, NULL, $this->last_service->ReadOnly);

			// next_service
			$this->next_service->SetDbValueDef($rsnew, $this->next_service->CurrentValue, NULL, $this->next_service->ReadOnly);

			// prod_capacity
			$this->prod_capacity->SetDbValueDef($rsnew, $this->prod_capacity->CurrentValue, NULL, $this->prod_capacity->ReadOnly);

			// uom
			$this->uom->SetDbValueDef($rsnew, $this->uom->CurrentValue, NULL, $this->uom->ReadOnly);
...
// Create page object
if (!isset($fixed_asset_edit)) $fixed_asset_edit = new cfixed_asset_edit();

// Page init
$fixed_asset_edit->Page_Init();

// Page main
$fixed_asset_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$fixed_asset_edit->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var fixed_asset_edit = new ew_Page("fixed_asset_edit");
fixed_asset_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = fixed_asset_edit.PageID; // For backward compatibility

// Form object
var ffixed_assetedit = new ew_Form("ffixed_assetedit");

// Validate form
ffixed_assetedit.Validate = function() {
...
ffixed_assetedit.Form_CustomValidate = 
...
ffixed_assetedit.ValidateRequired = true;
<?php } else { ?>
ffixed_assetedit.ValidateRequired = false; 
<?php } ?>
...
<?php $fixed_asset_edit->ShowPageHeader(); ?>
<?php
$fixed_asset_edit->ShowMessage();
?>
<form name="ffixed_assetedit" id="ffixed_assetedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($fixed_asset_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $fixed_asset_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $fixed_asset->fasset_id->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_fasset_id" name="x_fasset_id" id="x_fasset_id" value="<?php echo ew_HtmlEncode($fixed_asset->fasset_id->CurrentValue) ?>">
...
<script type="text/javascript">
ffixed_assetedit.Init();
</script>
<?php
$fixed_asset_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$fixed_asset_edit->Page_Terminate();
?>
