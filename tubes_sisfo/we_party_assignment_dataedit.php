...
$we_party_assignment_data_edit = NULL; // Initialize page object first

class cwe_party_assignment_data_edit extends cwe_party_assignment_data {
	...
	// Page object name
	var $PageObjName = 'we_party_assignment_data_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->wepad_id->CurrentValue == "")
			$this->Page_Terminate("we_party_assignment_datalist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->fk_wepad->SetDbValueDef($rsnew, $this->fk_wepad->CurrentValue, NULL, $this->fk_wepad->ReadOnly);

			// fk_wepad2
			$this->fk_wepad2->SetDbValueDef($rsnew, $this->fk_wepad2->CurrentValue, NULL, $this->fk_wepad2->ReadOnly);

			// we_role_type
			$this->we_role_type->SetDbValueDef($rsnew, $this->we_role_type->CurrentValue, NULL, $this->we_role_type->ReadOnly);
			...
			// com
			$this->com->SetDbValueDef($rsnew, $this->com->CurrentValue, NULL, $this->com->ReadOnly);
...
// Create page object
if (!isset($we_party_assignment_data_edit)) $we_party_assignment_data_edit = new cwe_party_assignment_data_edit();

// Page init
$we_party_assignment_data_edit->Page_Init();

// Page main
$we_party_assignment_data_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$we_party_assignment_data_edit->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var we_party_assignment_data_edit = new ew_Page("we_party_assignment_data_edit");
we_party_assignment_data_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = we_party_assignment_data_edit.PageID; // For backward compatibility

// Form object
var fwe_party_assignment_dataedit = new ew_Form("fwe_party_assignment_dataedit");

// Validate form
fwe_party_assignment_dataedit.Validate = function() {
...
fwe_party_assignment_dataedit.Form_CustomValidate = 
...
fwe_party_assignment_dataedit.ValidateRequired = true;
<?php } else { ?>
fwe_party_assignment_dataedit.ValidateRequired = false; 
<?php } ?>
...
<?php $we_party_assignment_data_edit->ShowPageHeader(); ?>
<?php
$we_party_assignment_data_edit->ShowMessage();
?>
<form name="fwe_party_assignment_dataedit" id="fwe_party_assignment_dataedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($we_party_assignment_data_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $we_party_assignment_data_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $we_party_assignment_data->wepad_id->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_wepad_id" name="x_wepad_id" id="x_wepad_id" value="<?php echo ew_HtmlEncode($we_party_assignment_data->wepad_id->CurrentValue) ?>">
...
<script type="text/javascript">
fwe_party_assignment_dataedit.Init();
</script>
<?php
$we_party_assignment_data_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$we_party_assignment_data_edit->Page_Terminate();
?>
