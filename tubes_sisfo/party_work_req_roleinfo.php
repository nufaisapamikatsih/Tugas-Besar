...
$party_work_req_role = NULL;

//
// Table class for party_work_req_role
//
class cparty_work_req_role extends cTable {
	var $pwrr_id;
	var $fk_pwrr;
	var $fk_pwrr2;
	var $fk_pwrr3;
	...
	function __construct() {
		...
		$this->TableVar = 'party_work_req_role';
		$this->TableName = 'party_work_req_role';
		...
		// pwrr_id
		$this->pwrr_id = new cField('party_work_req_role', 'party_work_req_role', 'x_pwrr_id', 'pwrr_id', '`pwrr_id`', '`pwrr_id`', 200, -1, FALSE, '`pwrr_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['pwrr_id'] = &$this->pwrr_id;

		// fk_pwrr
		$this->fk_pwrr = new cField('party_work_req_role', 'party_work_req_role', 'x_fk_pwrr', 'fk_pwrr', '`fk_pwrr`', '`fk_pwrr`', 200, -1, FALSE, '`fk_pwrr`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['fk_pwrr'] = &$this->fk_pwrr;

		// fk_pwrr2
		$this->fk_pwrr2 = new cField('party_work_req_role', 'party_work_req_role', 'x_fk_pwrr2', 'fk_pwrr2', '`fk_pwrr2`', '`fk_pwrr2`', 200, -1, FALSE, '`fk_pwrr2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['fk_pwrr2'] = &$this->fk_pwrr2;

		// fk_pwrr3
		$this->fk_pwrr3 = new cField('party_work_req_role', 'party_work_req_role', 'x_fk_pwrr3', 'fk_pwrr3', '`fk_pwrr3`', '`fk_pwrr3`', 200, -1, FALSE, '`fk_pwrr3`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['fk_pwrr3'] = &$this->fk_pwrr3;

		// from_date
		$this->from_date = new cField('party_work_req_role', 'party_work_req_role', 'x_from_date', 'from_date', '`from_date`', '`from_date`', 200, -1, FALSE, '`from_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['from_date'] = &$this->from_date;

		// thru_date
		$this->thru_date = new cField('party_work_req_role', 'party_work_req_role', 'x_thru_date', 'thru_date', '`thru_date`', '`thru_date`', 200, -1, FALSE, '`thru_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`party_work_req_role`";
	}
	...
	// Update Table
	var $UpdateTable = "`party_work_req_role`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('pwrr_id', $rs))
				ew_AddFilter($where, ew_QuotedName('pwrr_id') . '=' . ew_QuotedValue($rs['pwrr_id'], $this->pwrr_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`pwrr_id` = '@pwrr_id@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		$sKeyFilter = str_replace("@pwrr_id@", ew_AdjustSql($this->pwrr_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "party_work_req_rolelist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("party_work_req_roleview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("party_work_req_roleview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "party_work_req_roleadd.php?" . $this->UrlParm($parm);
		else
			return "party_work_req_roleadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("party_work_req_roleedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("party_work_req_roleadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("party_work_req_roledelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->pwrr_id->CurrentValue)) {
			$sUrl .= "pwrr_id=" . urlencode($this->pwrr_id->CurrentValue);
		} else {
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->pwrr_id->Exportable) $Doc->ExportCaption($this->pwrr_id);
					if ($this->fk_pwrr->Exportable) $Doc->ExportCaption($this->fk_pwrr);
					if ($this->fk_pwrr2->Exportable) $Doc->ExportCaption($this->fk_pwrr2);
					if ($this->fk_pwrr3->Exportable) $Doc->ExportCaption($this->fk_pwrr3);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->pwrr_id->Exportable) $Doc->ExportField($this->pwrr_id);
						if ($this->fk_pwrr->Exportable) $Doc->ExportField($this->fk_pwrr);
						if ($this->fk_pwrr2->Exportable) $Doc->ExportField($this->fk_pwrr2);
						if ($this->fk_pwrr3->Exportable) $Doc->ExportField($this->fk_pwrr3);
