...
$time_sheet_entry_view = NULL; // Initialize page object first

class ctime_sheet_entry_view extends ctime_sheet_entry {
	...
	// Page object name
	var $PageObjName = 'time_sheet_entry_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["tse_id"] <> "") {
			$this->RecKey["tse_id"] = $_GET["tse_id"];
			$KeyUrl .= "&amp;tse_id=" . urlencode($this->RecKey["tse_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["tse_id"] <> "") {
				...
				$this->RecKey["tse_id"] = $this->tse_id->QueryStringValue;
			} else {
				$sReturnUrl = "time_sheet_entrylist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "time_sheet_entrylist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "time_sheet_entrylist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($time_sheet_entry_view)) $time_sheet_entry_view = new ctime_sheet_entry_view();

// Page init
$time_sheet_entry_view->Page_Init();

// Page main
$time_sheet_entry_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$time_sheet_entry_view->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var time_sheet_entry_view = new ew_Page("time_sheet_entry_view");
time_sheet_entry_view.PageID = "view"; // Page ID
var EW_PAGE_ID = time_sheet_entry_view.PageID; // For backward compatibility

// Form object
var ftime_sheet_entryview = new ew_Form("ftime_sheet_entryview");

// Form_CustomValidate event
ftime_sheet_entryview.Form_CustomValidate = 
...
ftime_sheet_entryview.ValidateRequired = true;
<?php } else { ?>
ftime_sheet_entryview.ValidateRequired = false; 
<?php } ?>
...
<?php $time_sheet_entry_view->ExportOptions->Render("body") ?>
<?php
	foreach ($time_sheet_entry_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $time_sheet_entry_view->ShowPageHeader(); ?>
<?php
$time_sheet_entry_view->ShowMessage();
?>
<form name="ftime_sheet_entryview" id="ftime_sheet_entryview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($time_sheet_entry_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $time_sheet_entry_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="time_sheet_entry">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($time_sheet_entry->tse_id->Visible) { // tse_id ?>
	<tr id="r_tse_id">
		<td><span id="elh_time_sheet_entry_tse_id"><?php echo $time_sheet_entry->tse_id->FldCaption() ?></span></td>
		<td<?php echo $time_sheet_entry->tse_id->CellAttributes() ?>>
<span id="el_time_sheet_entry_tse_id" class="form-group">
<span<?php echo $time_sheet_entry->tse_id->ViewAttributes() ?>>
<?php echo $time_sheet_entry->tse_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($time_sheet_entry->ts_from->Visible) { // ts_from ?>
	<tr id="r_ts_from">
		<td><span id="elh_time_sheet_entry_ts_from"><?php echo $time_sheet_entry->ts_from->FldCaption() ?></span></td>
		<td<?php echo $time_sheet_entry->ts_from->CellAttributes() ?>>
<span id="el_time_sheet_entry_ts_from" class="form-group">
<span<?php echo $time_sheet_entry->ts_from->ViewAttributes() ?>>
<?php echo $time_sheet_entry->ts_from->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($time_sheet_entry->ts_thru->Visible) { // ts_thru ?>
	<tr id="r_ts_thru">
		<td><span id="elh_time_sheet_entry_ts_thru"><?php echo $time_sheet_entry->ts_thru->FldCaption() ?></span></td>
		<td<?php echo $time_sheet_entry->ts_thru->CellAttributes() ?>>
<span id="el_time_sheet_entry_ts_thru" class="form-group">
<span<?php echo $time_sheet_entry->ts_thru->ViewAttributes() ?>>
<?php echo $time_sheet_entry->ts_thru->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($time_sheet_entry->fk_tse->Visible) { // fk_tse ?>
	<tr id="r_fk_tse">
		<td><span id="elh_time_sheet_entry_fk_tse"><?php echo $time_sheet_entry->fk_tse->FldCaption() ?></span></td>
		<td<?php echo $time_sheet_entry->fk_tse->CellAttributes() ?>>
<span id="el_time_sheet_entry_fk_tse" class="form-group">
<span<?php echo $time_sheet_entry->fk_tse->ViewAttributes() ?>>
<?php echo $time_sheet_entry->fk_tse->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($time_sheet_entry->we_id->Visible) { // we_id ?>
	<tr id="r_we_id">
		<td><span id="elh_time_sheet_entry_we_id"><?php echo $time_sheet_entry->we_id->FldCaption() ?></span></td>
		<td<?php echo $time_sheet_entry->we_id->CellAttributes() ?>>
<span id="el_time_sheet_entry_we_id" class="form-group">
<span<?php echo $time_sheet_entry->we_id->ViewAttributes() ?>>
<?php echo $time_sheet_entry->we_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($time_sheet_entry->te_from->Visible) { // te_from ?>
	<tr id="r_te_from">
		<td><span id="elh_time_sheet_entry_te_from"><?php echo $time_sheet_entry->te_from->FldCaption() ?></span></td>
		<td<?php echo $time_sheet_entry->te_from->CellAttributes() ?>>
<span id="el_time_sheet_entry_te_from" class="form-group">
<span<?php echo $time_sheet_entry->te_from->ViewAttributes() ?>>
<?php echo $time_sheet_entry->te_from->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($time_sheet_entry->te_thru->Visible) { // te_thru ?>
	<tr id="r_te_thru">
		<td><span id="elh_time_sheet_entry_te_thru"><?php echo $time_sheet_entry->te_thru->FldCaption() ?></span></td>
		<td<?php echo $time_sheet_entry->te_thru->CellAttributes() ?>>
<span id="el_time_sheet_entry_te_thru" class="form-group">
<span<?php echo $time_sheet_entry->te_thru->ViewAttributes() ?>>
<?php echo $time_sheet_entry->te_thru->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($time_sheet_entry->hours->Visible) { // hours ?>
	<tr id="r_hours">
		<td><span id="elh_time_sheet_entry_hours"><?php echo $time_sheet_entry->hours->FldCaption() ?></span></td>
		<td<?php echo $time_sheet_entry->hours->CellAttributes() ?>>
<span id="el_time_sheet_entry_hours" class="form-group">
<span<?php echo $time_sheet_entry->hours->ViewAttributes() ?>>
<?php echo $time_sheet_entry->hours->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
ftime_sheet_entryview.Init();
</script>
<?php
$time_sheet_entry_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$time_sheet_entry_view->Page_Terminate();
?>
