...
$fixed_asset_assign = NULL;

//
// Table class for fixed_asset_assign
//
class cfixed_asset_assign extends cTable {
	var $faa_id;
	var $fk_faa;
	var $fk_faa2;
	var $from_date;
	...
	function __construct() {
		...
		$this->TableVar = 'fixed_asset_assign';
		$this->TableName = 'fixed_asset_assign';
		...
		// faa_id
		$this->faa_id = new cField('fixed_asset_assign', 'fixed_asset_assign', 'x_faa_id', 'faa_id', '`faa_id`', '`faa_id`', 200, -1, FALSE, '`faa_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['faa_id'] = &$this->faa_id;

		// fk_faa
		$this->fk_faa = new cField('fixed_asset_assign', 'fixed_asset_assign', 'x_fk_faa', 'fk_faa', '`fk_faa`', '`fk_faa`', 200, -1, FALSE, '`fk_faa`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['fk_faa'] = &$this->fk_faa;

		// fk_faa2
		$this->fk_faa2 = new cField('fixed_asset_assign', 'fixed_asset_assign', 'x_fk_faa2', 'fk_faa2', '`fk_faa2`', '`fk_faa2`', 200, -1, FALSE, '`fk_faa2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['fk_faa2'] = &$this->fk_faa2;

		// from_date
		$this->from_date = new cField('fixed_asset_assign', 'fixed_asset_assign', 'x_from_date', 'from_date', '`from_date`', '`from_date`', 200, -1, FALSE, '`from_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['from_date'] = &$this->from_date;

		// thru_date
		$this->thru_date = new cField('fixed_asset_assign', 'fixed_asset_assign', 'x_thru_date', 'thru_date', '`thru_date`', '`thru_date`', 200, -1, FALSE, '`thru_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['thru_date'] = &$this->thru_date;

		// comm
		$this->comm = new cField('fixed_asset_assign', 'fixed_asset_assign', 'x_comm', 'comm', '`comm`', '`comm`', 200, -1, FALSE, '`comm`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`fixed_asset_assign`";
	}
	...
	// Update Table
	var $UpdateTable = "`fixed_asset_assign`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('faa_id', $rs))
				ew_AddFilter($where, ew_QuotedName('faa_id') . '=' . ew_QuotedValue($rs['faa_id'], $this->faa_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`faa_id` = '@faa_id@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		$sKeyFilter = str_replace("@faa_id@", ew_AdjustSql($this->faa_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "fixed_asset_assignlist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("fixed_asset_assignview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("fixed_asset_assignview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "fixed_asset_assignadd.php?" . $this->UrlParm($parm);
		else
			return "fixed_asset_assignadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("fixed_asset_assignedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("fixed_asset_assignadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("fixed_asset_assigndelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->faa_id->CurrentValue)) {
			$sUrl .= "faa_id=" . urlencode($this->faa_id->CurrentValue);
		} else {
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->faa_id->Exportable) $Doc->ExportCaption($this->faa_id);
					if ($this->fk_faa->Exportable) $Doc->ExportCaption($this->fk_faa);
					if ($this->fk_faa2->Exportable) $Doc->ExportCaption($this->fk_faa2);
					...
					if ($this->comm->Exportable) $Doc->ExportCaption($this->comm);
				} else {
					if ($this->faa_id->Exportable) $Doc->ExportCaption($this->faa_id);
					if ($this->fk_faa->Exportable) $Doc->ExportCaption($this->fk_faa);
					if ($this->fk_faa2->Exportable) $Doc->ExportCaption($this->fk_faa2);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->faa_id->Exportable) $Doc->ExportField($this->faa_id);
						if ($this->fk_faa->Exportable) $Doc->ExportField($this->fk_faa);
						if ($this->fk_faa2->Exportable) $Doc->ExportField($this->fk_faa2);
						if ($this->from_date->Exportable) $Doc->ExportField($this->from_date);
						if ($this->thru_date->Exportable) $Doc->ExportField($this->thru_date);
						if ($this->comm->Exportable) $Doc->ExportField($this->comm);
					} else {
						if ($this->faa_id->Exportable) $Doc->ExportField($this->faa_id);
						if ($this->fk_faa->Exportable) $Doc->ExportField($this->fk_faa);
						if ($this->fk_faa2->Exportable) $Doc->ExportField($this->fk_faa2);
